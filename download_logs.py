...
def get_arguments():
    ...
    parser.add_argument('--min-duration', type=int, default=None,
                        help='Minimum duration of logs to download in seconds.')
    parser.add_argument('--max-duration', type=int, default=None,
                        help='Maximum duration of logs to download in seconds.')
...
def main():
    ...
    if args.print_entries:
        ...
        if args.min_duration is not None or args.max_duration is not None:
          filtered_entries = []
          for entry in db_entries_list:
            duration = entry.get('duration_s', 0) # Assuming 'duration' is in seconds
            if args.min_duration and duration < args.min_duration*60:
                continue
            if args.max_duration and duration > args.max_duration*60:
                continue
            filtered_entries.append(entry)
          db_entries_list = filtered_entries        
